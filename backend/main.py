...
def get_espn_league(league_id: str, espn_s2: str, swid: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Failed to connect to league: {str(e)}"
        )
...
async def connect_league(connection: LeagueConnection):
    ...
    try:
        league = get_espn_league(
            connection.league_id, connection.espn_s2, connection.swid
        )

        # Format league data
        league_data = {
            "id": league.league_id,
            "name": getattr(league, "settings", {}).get(
                "name", f"League {league.league_id}"
            ),
        ...
        return {"league": league_data, "teams": teams_data, "user_team": user_team}
...
async def analyze_trade(trade: TradeAnalysis):
    ...
    try:
        ...
        analysis = model_service.analyze_trade_impact(
            user_players_data, partner_players_data
        )
...
async def health_check():
    ...
    return {
        ...
        if model_service and model_service.feature_columns
        else 0,
    }
