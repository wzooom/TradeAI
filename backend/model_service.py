...
class PlayerValuationService:
    ...
    def _calculate_position_stats(self, df):
        ...
        try:
            ...
            position_groups = (
                df.groupby("position").mean() if "position" in df.columns else None
            )

            if position_groups is not None:
                for position in ["QB", "RB", "WR", "TE"]:
                    if position in position_groups.index:
                        self.position_stats[position] = position_groups.loc[
                            position
                        ].to_dict()

            logger.info(
                f"✅ Position statistics calculated for {len(self.position_stats)} positions"
            )
    ...
    def analyze_trade_impact(self, user_players: list, partner_players: list) -> dict:
        ...
        user_giving_value = sum(
            self.predict_fantasy_points(player) for player in user_players
        )
        user_receiving_value = sum(
            self.predict_fantasy_points(player) for player in partner_players
        )
        ...
        if user_net_change > 10:
            recommendation = (
                "Excellent trade for you! This significantly improves your team."
            )
        elif user_net_change > 5:
            recommendation = "Good trade for you. Consider accepting this offer."
        elif user_net_change > 0:
            recommendation = (
                "Slight advantage to you. Worth considering based on team needs."
            )
        elif user_net_change > -5:
            recommendation = (
                "Fairly even trade. Consider roster construction and bye weeks."
            )
        elif user_net_change > -10:
            recommendation = (
                "This trade favors your opponent. You might want to ask for more."
            )
        else:
