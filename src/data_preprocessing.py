...
df = df.merge(
    rosters[["player_id", "team", "season", "position", "status"]],
    on=["player_id", "season"],
    how="left",
    suffixes=("", "_roster"),
)
...
df = df.merge(
    teams[["team_abbr", "team_name", "team_conf", "team_division"]],
    left_on="team",
    right_on="team_abbr",
    how="left",
)
...
df["total_tds"] = (
    df.get("passing_tds", 0) + df.get("rushing_tds", 0) + df.get("receiving_tds", 0)
)
...
one_hot_cols = [
    ...
    "status",
    "team_conf",
    "team_division",  # <- team_name removed
]
...
df_encoded = pd.concat(
    [
        df.drop(columns=one_hot_cols).reset_index(drop=True),
...
print(
    f"Non-numeric columns left: {df_encoded.select_dtypes(exclude=['number']).columns.tolist()}"
)
...
df_scaled.to_csv(
    Path(__file__).parent.parent / "data" / "nfl_seasonal_preprocessed.csv", index=False
)

print("✅ Scaled dataset saved as nfl_seasonal_preprocessed.csv")
